...
int32 UPICOXRSystemAPI::PXR_SetScreenCastAudioOutput(EScreencastAudioOutput ScrrencastAudioOutput, int32 Ext)
{
	int32 Result = 0;
#if PLATFORM_ANDROID
	if (JNIEnv* Env = FAndroidApplication::GetJavaEnv())
	{
		...
		Result = FJavaWrapper::CallIntMethod(Env, FJavaWrapper::GameActivityThis, Method, (int)ScrrencastAudioOutput, Ext);
	}
#endif
	return Result;
}
