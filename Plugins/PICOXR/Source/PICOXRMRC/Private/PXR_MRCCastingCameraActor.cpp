...
void APICOXRMRC_CastingCameraActor::UpdateInGameCamPose()
{
	...
	else
	{
		...
		if (FPICOXRMRCModule::Get().PICOXRHMD && !bHasInitializedInGameCamOnce || MRState->bUpdateMRCCameraZ)
		{
			MRState->bUpdateMRCCameraZ = false;
			MRState->ZOffset = 0;
			...
			ExecuteOnRenderThread([this,&BG,&FG]()
				{
					ExecuteOnRHIThread([this,&BG,&FG]()
						{
							BG = BackgroundRenderTarget->GetResource()->TextureRHI;
							FG = ForegroundRenderTarget->GetResource()->TextureRHI;
						});
				});
#if PLATFORM_ANDROID
