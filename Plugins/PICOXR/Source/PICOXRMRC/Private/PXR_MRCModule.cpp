...
void FPICOXRMRCModule::OpenInGameCam()
{
	...
	{
		if (!IsValid(*ActorIt) && ActorIt->IsValidLowLevel())
		{
...
#endif
	FXRTrackingSystemDelegates::OnXRTrackingOriginChanged.AddRaw(this, &FPICOXRMRCModule::OnTrackingOriginChanged);
}
...
void FPICOXRMRCModule::OnWorldTick(UWorld* World, ELevelTick Tick, float Delta)
{
	...
	if (UpdateZCounter && GetMRCState())
	{
		GetMRCState()->bUpdateMRCCameraZ = true;
		--UpdateZCounter;
	}
}
...
void FPICOXRMRCModule::OnTrackingOriginChanged(const IXRTrackingSystem* TrackingSys)
{
	UpdateZCounter = 2;
}
