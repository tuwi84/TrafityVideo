...
int UPICOXRInputFunctionLibrary::PXR_SaveVibrateByCache(USoundWave* InSoundWave, EPICOXRVibrateController slot, EPICOXRChannelFlip slotConfig, EPICOXRCacheConfig enableV, int& SourceId) {
    ...
    if (InSoundWave == nullptr) {
       ...
       if (FPICOXRVersionHelper::IsThisVersionOrGreater(0x2000308))
       {
        ...
        Audio::FSoundWavePCMLoader SoundWaveLoader;
       	if (InSoundWave->RawPCMData == NULL)
       	{
       		FAudioDevice* AudioDevice = GEngine->GetMainAudioDeviceRaw();
       		if (AudioDevice)
       		{
       			EDecompressionType DecompressionType = InSoundWave->DecompressionType;
       			InSoundWave->DecompressionType = DTYPE_Native;

       			FName format = AudioDevice->GetRuntimeFormat(InSoundWave);

       			FByteBulkData* Bulk = InSoundWave->GetCompressedData(format, InSoundWave->GetPlatformCompressionOverridesForCurrentPlatform());
       			if (Bulk)
       			{
       				InSoundWave->InitAudioResource(*Bulk);
	
       				if (InSoundWave->DecompressionType != DTYPE_RealTime || InSoundWave->CachedRealtimeFirstBuffer == nullptr)
       				{
       					FAsyncAudioDecompress TempDecompress(InSoundWave, 128, AudioDevice);
       					TempDecompress.StartSynchronousTask();
       				}
       			}

       			InSoundWave->DecompressionType = DecompressionType;
       		}
       	}
        data = InSoundWave->RawPCMData;
