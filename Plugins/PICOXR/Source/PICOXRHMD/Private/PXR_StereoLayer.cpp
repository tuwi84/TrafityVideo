...
void FPICOXRStereoLayer::CreateUnderlayMesh(TArray<FVector>& Vertices, TArray<int32>& Triangles, TArray<FVector2D>& UV0)
{
	...
	else if (LayerDesc.HasShape<FEACLayer>())
	{
		...
		Vertices.Init(FVector::ZeroVector, 8);
		Vertices[0] = FVector(-1.0, -1.0, -1.0);
		Vertices[1] = FVector(-1.0, -1.0, 1.0);
		Vertices[2] = FVector(-1.0, 1.0, -1.0);
		Vertices[3] = FVector(-1.0, 1.0, 1.0);
		Vertices[4] = FVector(1.0, -1.0, -1.0);
		Vertices[5] = FVector(1.0, -1.0, 1.0);
		Vertices[6] = FVector(1.0, 1.0, -1.0);
		Vertices[7] = FVector(1.0, 1.0, 1.0);
...
{
	LayerDesc.SetShape<FEACLayer>(OverlapFactor, Subtype, LeftUVRect, RightUVRect, Offset, OffsetRot);
}
